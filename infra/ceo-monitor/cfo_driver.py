...
# aiyer@ only. The tenant's app-only access policy scopes the GRAPH_READER_* app
# to pganesharajah@ — and that is the ONLY mailbox it may read (excoboard@ and
# omni@ return "Blocked by tenant configured AppOnly AccessPolicy settings").
# So this driver reads GRAPH_READER_* and must never be pointed elsewhere.
...
def classify(cands):
    from core.ai_assist import reasoning_complete, gemini_complete
    cands=cands[:30]
    payload=json.dumps([{"i":i,"text":c["text"][:850]} for i,c in enumerate(cands)],ensure_ascii=False)
    verds,st=_run_model(reasoning_complete,payload)   # DeepSeek first (preferred)
    n_sig=sum(1 for v in (verds or {}).values() if v.get("significant"))
    n_strong=sum(1 for c in cands if _strong(c["text"]))
    if verds is None or (n_sig==0 and n_strong>0):    # DeepSeek dumb -> Gemini
        print("DeepSeek weak (%s, sig=%s, strong=%s) -> Gemini"%(st,n_sig,n_strong))
        g,gst=_run_model(gemini_complete,payload)
        if g is not None: verds=g; print("Gemini used:",gst)
...
def fetch_meetings():
    ...
    today=datetime.date.today(); start=today.isoformat()+"T00:00:00"; end=(today+datetime.timedelta(days=2)).isoformat()+"T00:00:00"
    # The GRAPH_READER_* app is scoped by tenant policy to pganesharajah@ ONLY,
    # so his is the one calendar this brief can read. The other two columns say
    # "not shared" rather than silently showing zero meetings, which would read
    # as "they have a free day".
    ...
    def _events(who):
        ...
        except Exception as ex: return [],getattr(ex,"code","")
    def _ext(e):
    ...
    tday={}; pending={}; tcounts={}; tomo_ext=[]
    for label,who in people:
        evs,err=_events(who)
        if err==403:
            tday[label]=[]; pending[label]=True; tcounts[label]=None; continue
        rows=[_fmt(e) for e in evs]
        tday[label]=[{"time":r["time"],"title":r["title"],"ext":r["ext"]} for r in rows if r["date"]==today_iso][:6]
    ...
    # No "still enabling" note here: this brief reads only the CFO's calendar by
    # design, so a line about someone else's would be permanently untrue.
    ...
    return {"today":tday,"pending":pending,"tomorrow_counts":tcounts,"tomorrow_headline":headline,"quip":quip}
...
d=E.Digest(date=datetime.date.today().strftime("%A, %d %B %Y"),
...
SUBJECT = "CFO Omni Brief - " + datetime.date.today().strftime("%A, %d %B %Y")
