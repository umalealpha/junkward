...
def score_item(item: Item) -> None:
    ...
    if (_CFO_ADDR in (getattr(item, "from_addr", "") or "").lower()
...
def render_one_thing(item):
    if not item:
        return ""
    rec = (f' <strong style="color:{GOLD};">Recommended:</strong> {_esc(item.rec_owner_label)} &mdash; {_esc(item.rec_text)}') if item.rec_text else ""
    return (
        ...
        f'<tr><td style="padding:18px 22px 4px 22px; font-family:{SERIF}; font-style:italic; color:{GOLD}; '
        'font-size:11px; letter-spacing:4px; text-transform:uppercase; line-height:16px;">The One Thing Today</td></tr>'
        f'<tr><td style="padding:6px 22px 0 22px; font-family:{SERIF}; color:{NAVY}; font-size:16px; '
        f'font-weight:bold; line-height:23px;">{_esc(item.polished_matter or item.title)}</td></tr>'
        f'<tr><td style="padding:8px 22px 18px 22px; font-family:{SANS}; color:{BODY}; font-size:13px; '
        f'line-height:19px;">{_esc(why_line(item))}{rec}</td></tr>'
        '</table></td></tr>')
...
def render_money_lens(lens):
    ...
    claims_html = ""
    large = lens.get("large_claims") or []
    if large:
        claim_rows = ""
        for cl in large:
            claim_rows += (
        ...
        claims_html = (
            ...
            'text-transform:uppercase; padding:14px 0 8px 0;">3 LARGEST NEW CLAIMS (last 10 days)</div>'
            '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">'
            f'{claim_rows}</table>')
...
def _mtg_col(name, role, count, evs, pending, accent):
    header_extra = (f'<br><span style="color:{ORANGE}; letter-spacing:1px; font-weight:normal;">{count} today</span>') if (count is not None and not pending) else ""
    ...
    if pending:
        body = (f'<tr><td style="padding:10px 12px; font-family:{SANS}; color:{FAINT}; font-size:11px; '
                'font-style:italic; line-height:16px;">Calendar access still enabling</td></tr>')
    else:
        body = ""
        if not evs:
            body = (f'<tr><td style="padding:10px 12px; font-family:{SANS}; color:{FAINT}; font-size:11px; '
...
def render_meetings(meetings, columns=None):
    ...
    for i, (key, role) in enumerate(columns[:3]):
        ...
        count = None if (i == 1 and pending.get(key)) else len(tday.get(key, []))
        cols.append(_mtg_col(label, role, count, tday.get(key, []),
                             pending.get(key), ORANGE if i == 0 else NAVY))
    while len(cols) < 3:
        cols.append(_mtg_col("", "", None, [], None, NAVY))
...
def render_html(digest, meetings=None, events=None, waiting=None, lens=None,
    ...
    # With a SINGLE matter the box and the card underneath it are word-for-word
    # the same, so the CEO reads the same issue twice (CFO, 11-Sep-2026). The
    # card is the one that carries the escalate buttons, so the box stands down.
    one_thing = digest.items[0] if len(digest.items) > 1 else None
    rows = (
        render_header(digest, band=band)
        + render_title_block(digest)
        + render_money_lens(lens)
        + (render_one_thing(one_thing) if one_thing else "")
