...
def realpay_collections_dashboard(request):
    ...
    # ---- LIVE from Graphite (primary) ----------------------------------------
    # The collected instalments Graphite pulls through RealPay are the source of
    # truth, and never go stale. When the read-only bridge is wired (prod) we read
    # them live and retire the manual Excel upload; the uploaded-row path below is
    # only the dev fallback where Graphite is not reachable.
    from . import graphite_feed
    ...
    base = RealPayTransaction.objects.filter(source=RealPayTransaction.Source.BILLING)
    if start:
        base = base.filter(txn_date__gte=start)
    if end:
        base = base.filter(txn_date__lte=end)
