        ...
        with open(path, encoding='utf-8-sig', errors='ignore', newline='') as f:
            for r in csv.DictReader(f):
                ...
                rows.append(RealPayTransaction(
                    ...
                    txn_date=_date(r.get('Transaction Date')),
                    client_number=(r.get('ClientNumber') or '').strip()[:40],
                    client_name='',  # billing detail carries no name
                    product=(r.get('Product') or '').strip()[:40],
                    beneficiary_number=(r.get('BeneficiaryNumber') or '').strip()[:20],
                    contract_sequence=(r.get('ContractSequence') or '').strip()[:20],
                    inst_seq=(r.get('InstallmentSequence') or '').strip()[:20],
                    amount_requested=_dec(r.get('AmountRequested')),
                    collected_amount=_dec(r.get('AmountCollected')),
                    result_code=rc[:20],
                    result_code_norm=normalize_response_code(rc),
                    import_batch=batch,
                ))
                n += 1
        self._replace(RealPayTransaction.Source.BILLING, rows, batch, replace_from)
        self.stdout.write(self.style.SUCCESS(f'Client Billing rows loaded: {n}'))
