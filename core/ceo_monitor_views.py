...
#
# PAYMENT TASKS ARE EXCLUDED BY DESIGN. taskboard.services.complete_task treats
# completing a `payment_request` task as the payment itself - it advances the
# linked PaymentRequest to PAID and is guarded by the PAY-DUP-01 duplicate
# checks and the per-line hold rules. That click must stay in Omni where those
# controls and a typed completion note apply; it must never hang off a link in
# an email. is_decidable_task() is the single gate, and it fails CLOSED.
# ---------------------------------------------------------------------------

_DECIDE_SALT = "ceo-monitor-decide"

#: Task sources that must be decided in Omni, never from the email.
...
def is_decidable_task(task) -> bool:
    """True only when a task is safe to decide from the email.

    Fails closed: anything that is not an OmniTask, anything financial,
    anything already finished, and anything whose shape we cannot read stays
    in Omni.
    ...
    if not isinstance(task, OmniTask):
        return False
    if (getattr(task, "source", "") or "") in FINANCIAL_TASK_SOURCES:
        return False
