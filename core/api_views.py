...
class UserProfileViewSet(viewsets.ModelViewSet):
    ...
    def _require_admin(self):
        ...
        if not (profile and profile.can_administer_users):
            raise PermissionDenied(
                "Administrator privileges are required. Only the CFO or a user "
                "marked is_administrator can manage profiles."
            )
...
def user_company_access_admin(request):
    ...
    if request.method == 'POST':
        ...
        comp = (Company.objects.filter(id=comp_id).first()
                if comp_id else None) or Company.objects.filter(code__iexact=str(comp_id)).first()
        if not comp:
            return _Resp({'detail': 'Company not found.'}, status=404)
    ...
    if not u:
        return _Resp({'detail': 'User not found.'}, status=404)
    comp = (Company.objects.filter(id=cident).first()
            or Company.objects.filter(code__iexact=cident).first())
    if not comp:
        return _Resp({'detail': 'Company not found.'}, status=404)
...
def user_company_access_bulk(request):
    ...
    misses_c = []
    for s in c_idents:
        c = (Company.objects.filter(id=s).first()
             or Company.objects.filter(code__iexact=s).first())
        (companies if c else misses_c).append(c or s)
    companies = [c for c in companies if c is not None]

    if not users or not companies:
        return _Resp({
            'detail': 'No valid users or companies resolved.',
