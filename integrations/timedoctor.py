...
class TimeDoctorClient:
    ...
    def worklog(self, day_from, day_to, user_ids=None) -> List[list]:
        ...
        params = {'from': self._fmt_bound(day_from), 'to': self._fmt_bound(day_to), 'limit': 50000}
        if user_ids:
            params['user'] = ','.join(str(u) for u in user_ids)
        return self._get('/api/1.0/activity/worklog', params) or []
    ...
    def timeuse(self, day_from, day_to, user_ids=None) -> List[list]:
        ...
        if user_ids:
            params['user'] = ','.join(str(u) for u in user_ids)
        return self._get('/api/1.0/activity/timeuse', params) or []
