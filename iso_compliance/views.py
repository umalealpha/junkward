...
class RunAuditView(APIView):
    ...
    def post(self, request):
        if not _can_run_audit(request.user):
            return Response(
                {'detail': 'Only the CFO, superusers, or iso_auditor group can run audits.'},
...
class _RWPermission(IsAuthenticated):
    """Anyone authenticated can read; writes require CFO/super/iso_auditor."""
