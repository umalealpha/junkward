...
def pos_pending_for(user) -> dict:
    """POs waiting for THIS person to approve, by their finance role.
    CFO → pending-CFO; FM/FC → pending-FM. Others → none. Best-effort."""
    out = {'count': 0, 'items': []}
    try:
        ...
        if title == UserProfile.Title.CFO:
            qs = PurchaseOrder.objects.filter(status=S.PENDING_CFO_APPROVAL)
        elif title in (UserProfile.Title.FINANCE_MANAGER, UserProfile.Title.FINANCIAL_CONTROLLER):
            qs = PurchaseOrder.objects.filter(status=S.PENDING_FM_APPROVAL)
        else:
            return out
        out['count'] = qs.count()
        out['items'] = [po.po_number for po in qs.order_by('-created_at')[:5]]
    except Exception:    # noqa: BLE001
        pass
    return out
...
def build_morning_html(*, name, day, required, tracked_yesterday, week_hours,
                       rank, team_size, winner_name, leave, tasks, announcements, pos,
    ...
    pos_html = ''
    if pos['count']:
        lst = ', '.join(esc(str(x)) for x in pos['items'])
        more = '' if pos['count'] <= len(pos['items']) else f' (+{pos["count"] - len(pos["items"])} more)'
        pos_html = section('🧾', 'POs waiting for your approval',
                           f'<div style="font-size:13px;color:{INK}"><b>{pos["count"]}</b> to review: {lst}{more}</div>')
