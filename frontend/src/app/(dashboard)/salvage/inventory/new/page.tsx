 ...
 *
 * Phase 2 (deferred): photo upload UI wiring, write-off
 * classification fields, public buyer portal bidding, VIN search,
 * yard performance reports.
 */
...
export default function NewSalvageItemPage() {
  ...
  useEffect(() => {
    apiFetch<PaginatedRows<OptionRow>>('/salvage-brands/?page_size=200')
      .then((d) => setBrands(d.results || []))
      .catch(() => { /* dropdown stays empty — user can type vin manually */ })
    apiFetch<PaginatedRows<OptionRow>>('/salvage-categories/?page_size=200')
  ...
  useEffect(() => {
    ...
    apiFetch<PaginatedRows<OptionRow>>(`/salvage-models/?brand=${brand}&page_size=200`)
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!itemCode.trim() || !partName.trim()) {
      setErr('Item code and part name are required.')
      return
    }
    ...
    try {
      ...
      setOk(r)
      setTimeout(() => router.push('/salvage/inventory'), 1500)
    } catch (e2) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Item code *" theme={theme}>
                <input value={itemCode} onChange={e => setItemCode(e.target.value)}
                       placeholder="SLV-2026-0001" required {...inputProps(theme)} />
              </Field>
