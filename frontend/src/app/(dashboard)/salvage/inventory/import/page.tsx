 ...
 *   2. Validate the preview locally (item_code + part_name required,
 *      no client-side duplicates).
...
export default function ImportSalvageItemsPage() {
  ...
  async function handleFile(f: File | null) {
    ...
    try {
      ...
      if (!canonicalCols.has('item_code') || !canonicalCols.has('part_name')) {
        throw new Error(`File must include "item_code" and "part_name" columns. Found: ${headers.join(', ')}`)
      }
      ...
      const out: PreviewRow[] = raw.map((r, i) => {
        ...
        if (!code) warning = 'item_code is blank'
        else if (!name) warning = 'part_name is blank'
        else if (seen.has(code)) warning = `duplicate item_code "${code}" in file`
        else seen.add(code)
        return {
          row_number:    i + 2,
          item_code:     code,
  ...
  async function confirm() {
    ...
    try {
      ...
      fd.append('file', file)
      if (selectedId) fd.append('company', selectedId)
  ...
  function downloadTemplate() {
    const csv =
      ...
      'SLV-2026-0001,Front bumper assembly,CLM-2026-0042,fair,available,5000,3500,3500,Yard A Bay 12,2026-05-18,Toyota Hilux 2018\n' +
      'SLV-2026-0002,Engine block,CLM-2026-0042,good,available,15000,11000,11000,Yard A Bay 12,2026-05-18,\n'
          ...
          <p className="text-xs leading-relaxed" style={{ color: theme.text }}>
            ...
            {selected && (
              <span className="block mt-1">
                Items will land in company: <strong>{selected.code || selected.name}</strong>.
              </span>
            )}
          </p>
        </div>
