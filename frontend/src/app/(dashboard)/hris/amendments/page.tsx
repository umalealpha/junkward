    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className={`rounded-xl border p-5 mb-8 ${card}`}>
          ...
          <select value={eid} onChange={e => setEid(e.target.value)} className={`w-full rounded-lg border px-3 py-2 mb-4 ${inp}`}>
            <option value="">— select an employee —</option>
            {employees.map(e => <option key={e.eid} value={e.eid}>{e.nm} · {e.ps} · {e.dp}</option>)}
          </select>
