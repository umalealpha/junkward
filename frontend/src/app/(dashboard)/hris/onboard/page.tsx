...
export default function HrisOnboardPage() {
  ...
  async function submit() {
    if (!fullName.trim() || !email.trim() || !companyId || !hireDate) {
      setMsg({ ok: false, text: 'Full name, work email, entity and start date are required.' }); return
    }
    ...
    try {
      const response = await authedHrisFetch('/hris/api/onboard-employee/', {
        ...
        body: JSON.stringify({
          ...
          company_id: companyId, department: department.trim(), job_title: jobTitle.trim(),
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="rounded-2xl p-5 space-y-4 max-w-4xl" style={card}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Field label="Department" value={department} onChange={setDepartment} placeholder="e.g. Health Insurance" inp={inp} lbl={lbl} />
          ...
          {queue.map(item => <div key={item.id} className="rounded-xl p-4 space-y-3" style={card}>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
              <div>
                ...
                <p className="text-xs mt-1" style={lbl}>{item.job_title || 'Job title not set'}{item.department ? ` · ${item.department}` : ''} · submitted by {item.maker}</p>
              </div>
