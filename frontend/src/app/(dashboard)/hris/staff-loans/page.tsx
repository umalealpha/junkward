 ...
 *   • HR disburses — which creates the payroll loan and posts the GL entry.
 *
...
export default function StaffLoansPage() {
  ...
  const hrQueue = loans.filter(l => l.can_disburse)
    ...
    <div>
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-6">
        ...
        {hrQueue.length > 0 && (
          <Card><CardContent className="p-5">
            <div className="font-medium mb-2 flex items-center gap-2">
              <Banknote className="h-4 w-4 text-emerald-600" /> To pay out ({hrQueue.length})
            </div>
            {hrQueue.map(l => (
              <LoanRow key={l.id} l={l} actions={
