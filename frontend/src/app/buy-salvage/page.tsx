      ...
      {items.map(i => (
        <button key={i.id} onClick={() => onPick(i)}
          ...
          <div style={{ height: 160, background: '#0D1B2A', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#334' }}>
            {i.primary_image ? <Image src={i.primary_image} alt={i.part_name} width={260} height={160} style={{ objectFit: 'cover' }} /> : 'No photo'}
          </div>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <div style={{ background: '#152033', borderRadius: 10, padding: 24 }}>
          {i.primary_image
            ? <Image src={i.primary_image} alt={i.part_name} width={500} height={400} style={{ objectFit: 'cover', borderRadius: 8 }} />
