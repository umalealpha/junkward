 ...
 * One glance: who is in, who is on leave, who has gone dark on Time Doctor,
...
interface Glance { as_of: string; reports: Report[]; counts: { in: number; on_leave: number; dark: number; overdue: number } }
...
export default function TeamScreen() {
  ...
  const counts = glance?.counts ?? { in: 0, on_leave: 0, dark: 0, overdue: 0 }
  ...
  return (
    ...
    <div style={{ background: C.surface, minHeight: '100vh', fontFamily: sans }}>
      ...
      {!noTeam && (
        <>
          <section aria-label="Team today" style={{ ...card, padding: '14px 12px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 4 }}>
            <Count n={counts.in} label="in" color={GREEN} />
            <Count n={counts.on_leave} label="on leave" color={AMBER} />
            <Count n={counts.dark} label="dark" color={RED} />
            <Count n={counts.overdue} label="overdue" color={RED} />
          </section>
            ...
            {reports.map((r, i) => (
              <button key={r.employee_id} onClick={() => setOpenFor(r)} className="oa-press"
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }} aria-hidden>
                  <Dot color={r.online ? GREEN : GREY} title={r.online ? 'in' : 'not seen'} />
                  {r.on_leave_today && <Dot color={AMBER} title="on leave" />}
                  {r.dark_days_7 > 0 && <Dot color={RED} title="dark" />}
            ...
            <Section title="Explanations to review">
              <Link href={`${base}/review-explanations`} className="oa-press" style={rowLink}>
                ...
                <span style={{ flex: 1, fontWeight: 600, fontSize: 15, color: C.ink }}>
                  {explanations === 0 ? 'Nothing waiting on you' : `${explanations} lost-hours explanation${explanations === 1 ? '' : 's'} to decide`}
                </span>
...
function personStatus(r: Report): string {
  const bits = [r.online ? 'in' : 'not seen', r.on_leave_today ? 'on leave' : '', r.dark_days_7 ? `dark ${r.dark_days_7} day${r.dark_days_7 === 1 ? '' : 's'}` : '',
                r.overdue_tasks ? `${r.overdue_tasks} overdue` : ''].filter(Boolean)
  return bits.join(', ')
}
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={report.name} onClick={onClose}
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        {report.dark_days_7 > 0 && (
          <Note tone="red">Dark on Time Doctor {report.dark_days_7} day{report.dark_days_7 === 1 ? '' : 's'} this week — hours short with no accepted explanation yet.</Note>)}
