...
export async function matchStatementLine(
  lineId: string,
  data: { payment_id?: string; journal_entry_id?: string }
): Promise<BankStatementLine> {
  return apiFetch<BankStatementLine>(`/bank-statement-lines/${lineId}/match/`, {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export type PaymentCategory = 'claim' | 'supplier' | 'vendor' | 'petty_cash' | 'premium_refund' | 'unicoin' | 'quantum' | 'rsa' | 'veritas' | 'adh' | 'gce' | 'other' | ''
...
export async function getPaymentRequests(all = false): Promise<{ requests: PaymentRequestRow[]; is_cfo: boolean; is_first_approver?: boolean; showing?: string; bank_rejected_count?: number }> {
  return apiFetch(`/payment-requests/${all ? '?all=1' : ''}`)
}
...
export interface ClaimsRegisterRow {
  ...
  date_of_loss: string | null; damage_cause?: string;
