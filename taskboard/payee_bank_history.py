...
def last_known_bank(payee: str, *, exclude_pk=None, exact=False) -> dict | None:
    ...
    for row in qs.iterator():
        if _matches(normalise_payee(row['payee'] or row['account_name'])):
            return {
...
def bank_change_warning(payee: str, account_number: str, *,
    ...
    return {
        ...
        'detail': (
            f'Why are you doing this? You paid {payee} into an account ending '
            f'{old_tail} before, and this request says {new_tail}. '
...
def bank_details_changed(payee: str, *, bank_name: str = '', branch_code: str = '',
    ...
    return {
        ...
        'detail': (
            ...
            f'for {payee} (from {known["source_label"]}).\n\n'
