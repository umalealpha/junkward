...
def load_request_to_fnb(pr, releaser) -> dict:
    ...
    def _record(loaded: bool, reason: str, batch=None, hold_claim: bool = False):
        ...
        """
        pr.fnb_load_error = '' if loaded else reason[:2000]
    ...
    if pr.fnb_batch_id:
        return _record(True, '', pr.fnb_batch)
    ...
    try:
        ...
        if failed:
            ...
            pr.fnb_load_error = (
                ...
                f'Failed: {names[:1500]}. Load the rest by hand or try again.'
            )
            pr.save(update_fields=['fnb_load_error', 'updated_at'])
