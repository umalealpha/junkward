...
def _category_label(cat: str) -> str:
    return dict(PaymentRequest.Category.choices).get((cat or '').strip(), '')
...
def _same_loader_recent_same_amount(lines, *, currency: str, user, hours: int = 48) -> list:
    ...
    from .payment_duplicates import DEAD_STATUSES, _money
    ...
    recent = (PaymentRequest.objects
              .filter(created_by=user, currency=currency,
...
def payment_requests(request):
    ...
    if request.method == 'GET':
        ...
        bank_rejected_count = qs.filter(fnb_batch__status='failed').count()
        ...
        # Pagination. The action screens keep the historic 200-row cap; the
        # register paginates so the full history is reachable in pages instead of
        # silently truncated at 200 (Kago 2026-08-29).
        reg_total = None
        reg_limit = reg_offset = None
        if can_register:
            try:
                reg_limit = min(max(int(request.query_params.get('limit', 100)), 1), 500)
                reg_offset = max(int(request.query_params.get('offset', 0)), 0)
            except (TypeError, ValueError):
                return Response({'detail': 'limit/offset must be whole numbers.'}, status=400)
            reg_total = ordered.count()
            _page = ordered[reg_offset:reg_offset + reg_limit]
        else:
            _page = ordered[:200]
        ...
        rows = [{
            ...
            'bank_rejected': bool(p.fnb_batch_id and getattr(p.fnb_batch, 'status', '') == 'failed'),
        ...
        if can_register:
            payload['register'] = {
                'total': reg_total, 'limit': reg_limit, 'offset': reg_offset,
                'returned': len(rows),
                'has_more': (reg_offset + len(rows)) < reg_total,
            }
    ...
    if not petty_exempt:
        ...
        if missing:
            ...
            soft_reasons.append(
                ...
                'committee confirms.')
        # FNB is the only bank we can pay without a branch code (it falls back
        # to the FNB-to-FNB branch). Anyone else needs one or the bank rejects.
        if branch_in == '' and acct_in and not re.search(r'fnb|first\s*national', bank_in, re.I):
            soft_reasons.append(
                f'PAY-BANK-02: {_no_marker(bank_in) or "this bank"} usually needs a branch code '
                '(only FNB may be left blank) — the committee confirms.')
...
def _mark_paid_from_bank(p, user, reason, *, now=None):
...
def payment_request_reconcile_fnb_list(request):
    ...
    def _row(p, in_list):
        ...
        return {
            ...
            'bank_rejected': bool(b and getattr(b, 'status', '') == 'failed'),
    ...
    close_total = sum((Decimal(str(p.total)) for p in to_close), Decimal('0.00'))
    rejected = [p for p in to_close if p.fnb_batch_id and getattr(p.fnb_batch, 'status', '') == 'failed']
    not_loaded = [p for p in to_close if not p.fnb_batch_id]
    ...
    if mode != 'apply':
        return Response({
            ...
            'would_close': [_row(p, False) for p in to_close],
...
def payment_request_detail(request, req_id):
    ...
    return Response({
        ...
        'bank_rejected':  bool(p.fnb_batch_id and getattr(p.fnb_batch, 'status', '') == 'failed'),
        'bank_reject_reason': (
            (getattr(p.fnb_batch, 'failure_reason', '') or '')
            if p.fnb_batch_id and getattr(p.fnb_batch, 'status', '') == 'failed' else ''
        ) or (p.fnb_load_error or ''),
