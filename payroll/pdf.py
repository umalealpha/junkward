    ...
    if foreign:
        ...
        if not b.earnings and payslip.source_gross is not None:
            ...
            if other > 0:
                ...
                total_ded = paye
        rate = payslip.fx_rate_to_bwp or Decimal('1')
        fx_note = (f'Paid in {ccy}. Group reporting converts at '
                   f'{rate.normalize()} BWP per 1 {ccy} '
                   f'(BWP gross {Decimal(str(payslip.gross_amount or 0)):,.2f}).')
