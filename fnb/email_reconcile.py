...
def _batch_matches(email_ref: str, request: dict) -> bool:
    ...
    payment under (PaymentRequest.fnb_batch.idempotency_key, e.g.
    'ALPHA-EFT-20260821-09da76bc3db645f8'). That id is globally unique, so it is
    the strongest possible match for an omni-submitted payment — and the only one
    available when the payee reference was never stored on the request."""
    bk = _norm(request.get('batch_key') or '')
    if len(bk) < 12:            # a real ALPHA-EFT id is far longer; guards blanks
        return False
    return bk in _norm(email_ref)
...
def open_reason(*, batch_status='', failure_reason='', has_batch=False,
                is_exception=False, email_confidence='none') -> tuple[str, str]:
