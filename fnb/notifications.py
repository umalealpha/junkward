...
def _persist_items(items: Iterable[dict]) -> int:
    """Upsert each notification by FNB's external id. Side-effects from
    downstream consumers (mark payment as paid, update batch status, etc.)
    are dispatched by signals on FNBWebhookEvent post_save."""
    inserted = 0
    for item in items:
        ...
        if not ext_id:
            ...
            continue
        # camt.054 derives type from the first Ntry's CdtDbtInd + BkTxCd
        ...
        _, was_created = FNBWebhookEvent.objects.update_or_create(
            external_id=ext_id,
            defaults={
