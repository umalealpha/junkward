...
class ComparisonTests(TestCase):
    ...
    def test_a_flat_mapping_charges_the_panel_fee_once_however_many_hours(self):
        """A flat external fee is a fee, not a rate. Multiplying it by our hours
        would invent a saving that grows with the time we spent."""
        _mapping('Initial consultation', 'flat', 500)
        f = _fee('Initial consultation', 200, 3)
        index = calc.mapping_index(LegalRateMapping.objects.all())
        self.assertEqual(calc.external_equivalent(f, index), D('500'))
        self.assertEqual(calc.internal_amount(f), D('600'))

    def test_a_per_hour_mapping_multiplies_by_the_hours_logged(self):
        _mapping('Legal research', 'per_hour', 2250)
        f = _fee('Legal research', 300, '0.75')
        index = calc.mapping_index(LegalRateMapping.objects.all())
        self.assertEqual(calc.external_equivalent(f, index), D('1687.50'))

    def test_a_service_with_no_mapping_has_no_equivalent_it_is_not_a_zero_saving(self):
        f = _fee('Something nobody mapped', 300, 2)
        index = calc.mapping_index(LegalRateMapping.objects.all())
        self.assertIsNone(calc.external_equivalent(f, index))
        line = calc.fee_line(f, index)
        self.assertFalse(line['mapped'])
        self.assertIsNone(line['saving'])

    def test_a_mapping_is_found_whatever_case_it_was_typed_in(self):
        _mapping('Legal research', 'per_hour', 2250)
        f = _fee('  LEGAL Research ', 300, 1)
        index = calc.mapping_index(LegalRateMapping.objects.all())
        self.assertEqual(calc.external_equivalent(f, index), D('2250'))

    def test_the_saving_is_measured_only_on_the_lines_that_have_a_comparison(self):
        """The unmapped line's internal cost must not be subtracted from an
        external total it has no part in — that understates the saving and hides
        that the map needs a line adding."""
        _mapping('Legal research', 'per_hour', 2250)
        mapped = _fee('Legal research', 300, 1)          # internal 300, external 2250
        _fee('Unmapped thing', 1000, 1)                  # internal 1000, no external
        index = calc.mapping_index(LegalRateMapping.objects.all())
        t = calc.totals_for_fees([mapped, *LegalFeeNote.objects.exclude(pk=mapped.pk)], index)
        self.assertEqual(t['internal'], D('1300'))
        self.assertEqual(t['mapped_internal'], D('300'))
        self.assertEqual(t['external'], D('2250'))
        self.assertEqual(t['saving'], D('1950'))
...
class ApiTests(TestCase):
    ...
    def test_the_overview_reports_the_month_asked_for_not_the_latest(self):
        ...
        self.assertEqual(body['month'], '2026-06')
        self.assertEqual(D(body['summary']['matter_billing']), D('300'))
        self.assertEqual(D(body['summary']['external_equivalent']), D('2250'))
    ...
    def test_total_value_delivered_adds_the_three_parts(self):
        _mapping('Legal research', 'per_hour', 2250)
        _fee('Legal research', 300, 1, when=date(2026, 7, 3))          # in-house saving 1950
        LegalInvoiceSaving.objects.create(
        ...
        body = self.client.get('/api/v1/bonu/legal/?month=2026-07').json()['summary']
        self.assertEqual(D(body['total_value']), D('6200'))
    ...
    def test_section_3_uses_the_demo_basis_the_report_downloads(self):
        ...
        _mapping('Legal research', 'per_hour', 2250)
        _fee('Legal research', 300, 1, when=date(2026, 7, 3))     # mapped: 300 vs 2250
        _fee('Unmapped thing', 1000, 1, when=date(2026, 7, 3))    # no comparison
        ...
        self.assertEqual(D(s['external_equivalent']), D('2250'))
        self.assertEqual(
            D(s['external_equivalent']) - D(s['matter_billing']),
            D(s['in_house_saving']), 'saving = external − ALL billing (the demo basis)')
        self.assertEqual(D(s['in_house_saving']), D('950'))          # 2250 − 1300
        # the per-matter comparison tab keeps its mapped-line lifetime basis
        self.assertEqual(D(s['lifetime_mapped_internal']), D('300'))
        self.assertEqual(
