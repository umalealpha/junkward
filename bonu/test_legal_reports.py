...
* the monthly in-house saving is external cost less ALL matter billing (the
  demo's basis) — not the mapped-only subset, which would read 4,730 here,
* the quarterly bonus is 2% of the WHOLE quarter's saving once the trigger is
  reached, and zero below it,
* an unmapped service carries no external equivalent and no saving,
...
class FeeNoteModelTests(TestCase):
    ...
    def test_external_total_counts_mapped_lines_only(self):
        self.assertEqual(self._model()['ext_total'], D('5000'))  # 500 + 4500

    def test_in_house_saving_is_external_less_all_billing(self):
        # The demo's basis: monthlyExternalTotal - monthlyFeeTotal. Mapped-only
        # internal (270) would give 4,730 — the wrong number.
        self.assertEqual(self._model()['in_house_saving'], D('4630'))  # 5000 - 370

    def test_monthly_bonus_is_held_to_the_cap(self):
        self.assertEqual(self._model()['bonus'], D('6500'))  # 7000 entered, capped

    def test_an_unmapped_line_has_no_external_and_no_saving(self):
        row = next(r for r in self._model()['diff_rows'] if r['description'] == 'Mystery service')
        self.assertIsNone(row['external'])
        self.assertIsNone(row['saving'])
...
class QuarterlyModelTests(TestCase):
    ...
    def test_illustrative_saving_uses_mapped_lines_only(self):
        self.assertEqual(self._model()['illustrative'], D('4730'))  # 300 + 4430

    def test_total_delivered_adds_the_three_strands(self):
        # 54000 (contractual) + 4730 (illustrative) + 6750 (advisory 3h * 2250)
        self.assertEqual(self._model()['total_delivered'], D('65480'))
...
class ReportEndpointTests(TestCase):
    ...
    def test_the_screen_figure_matches_the_downloaded_report(self):
        ...
        self.assertEqual(D(s['in_house_saving']), D('4630'))  # 5000 − 370, the demo basis
