...
def feenote_model(month: str, fees, savings, mappings, entered_bonus, settings) -> dict:
    ...
    for f in lines:
        ...
        diff_rows.append({
            ...
            'basis': 'Disbursement' if (m and m.is_disbursement) else 'Attorney hourly / flat fee',
            'internal': internal,
            'external': external,
...
def feenote_docx(m: dict) -> bytes:
    ...
    _docx_meta(doc, f"Internal costs are what Alpha Direct actually paid, handling these BONU "
                    ...
                    f"the same matters gone to an external panel attorney, at the Standard Rate of "
                    f"{money(m['external_rate'])}/hr for hourly-billed work (disbursement items — "
                    f"email, telephone — keep their own separate rates).")
    _docx_table(
        doc,
...
def feenote_pdf(m: dict) -> bytes:
    ...
    flow = [
        ...
        Paragraph(f"Internal costs are what Alpha Direct actually paid, handling these BONU matters "
                  ...
                  f"matters gone to an external panel attorney, at the Standard Rate of "
                  f"{money(m['external_rate'])}/hr for hourly-billed work (disbursement items — "
                  f"email, telephone — keep their own separate rates).", meta),
        _pdf_table(
            ['Service', 'Basis', 'Internal cost', 'External finance equiv.', 'Saving'],
            [[r['description'], r['basis'], money(r['internal']),
              '—' if r['external'] is None else money(r['external']),
